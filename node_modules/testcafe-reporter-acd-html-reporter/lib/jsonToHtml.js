...
module.exports = {
  ...
  getJsonAsHtml: function (json) {
    ...
    stepsArray.forEach(stepsData => {
      generatedReport += stepsData.steps.replace('<step>', `<div fixture="${stepsData.fixture}" test="${stepsData.test.replace(/"/g, '&#34;')}" screenshot="${stepsData.screenshot}" durationMs="${stepsData.durationMs}" userAgent="${stepsData.userAgent}">`);
      if (stepsData.stackTrace && stepsData.stackTrace.length) generatedReport += `<div traceFixture="${stepsData.fixture}" traceTest="${stepsData.test.replace(/"/g, '&#34;')}">${JSON.stringify(stepsData.stackTrace)}</div>`;
    });
  ...
  stepsToString: function (steps) {
    ...
    steps.forEach(step => {
      ...
      if (step.name) stepsString += `<div class="stepName">${step.name}</div>`;
